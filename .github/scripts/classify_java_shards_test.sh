...
# The edit these guards exist to catch — a cron pushed back past 19:00 IST, or a passthrough moved
# to the wrong service — touches compose and NOTHING else. Before this rule that classified to no
# shard at all, so EveningScheduleWindowTest and SwingCoverageGateDefaultTest never ran and the PR
# went green. backtest stays false deliberately: nothing in that shard reads the file.
