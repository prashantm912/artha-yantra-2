...
-- accrued in the row-store. AYDB-03 measured candles_5m alone at ~15 GB live, and the DB is at 91.5%
-- of the 50 GB review trigger. Compressing the caggs is the direct lever. This migration only
-- registers metadata + a background compression policy per cagg — it does NOT compress synchronously
-- (no compress_chunk loop), so the migration itself is fast and light; the policy jobs compress aged
-- chunks lazily on their own schedule.
--
-- CRITICAL ORDERING (TimescaleDB 2.17.2 — this is BEFORE 2.18.1, where compressed cagg regions still
-- CANNOT be refreshed): each policy's `compress_after` MUST be strictly greater than the matching
-- V004/V029 refresh policy's `start_offset`, or the refresh job would try to write a compressed chunk
-- and error. Refresh start_offsets today: 5m=1d, 15m=2d, 1h=7d, 1d=14d, 1w=60d. Every compress_after
-- below clears its start_offset with wide margin. The recent (uncompressed) window each policy leaves
-- hot is what the live SignalEngine + charts read at fine granularity; older chunks compress.
--
...
--     materialization or bulk compress here (the #2bE1 / candles_3m OOM lesson). Deploy carefully:
--     the live DB is near capacity, but the background jobs only ever REDUCE size.
