...
| ~~Upstox `/pcr` … `source.pcr` switch~~ ✅ **CLOSED 2026-08-03 — it was never dormant** | **CLOSED** | — | ⚠️ **This row was wrong in its premise.** The switch is not "dormant awaiting a flip": `ARTHA_MD_SOURCE_OPTIONANALYTICS=upstox` has been the DEPLOYED value for ≥28 days (`.env:56` + container env; already recorded at `docs/audits/2026-07-06-ui-data-correctness/options.md:17`). This row, like three others, read the YAML default `${…:native}` as the deployed value. The live market-hours check ran 2026-08-03: median PCR divergence **−0.01%** over 57 paired 3-minute buckets, max-pain identical **55/57**, and native already covers **100%** of the listed strike universe on all 7 NIFTY expiries — so the flip's stated rationale (fuller chain) is obsolete. Upstox additionally flickers **0.26%** between two backend replicas on the same bucket where native is byte-identical. **Owner decided 2026-08-03: KEEP `upstox`** — nothing is broken and it has served harmlessly for a month. Ledger row **H6** is the authority; PCR still feeds NO strategy (display-only, 5 consumers, all one React page). |
