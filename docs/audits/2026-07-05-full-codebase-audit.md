...
| H5 | Graduation P&L attributed to the ONE opening strategy — V026 `paper_positions.opening_signal_id` + fill-path stamp (first-open only; averaged/pyramid add keeps the original) + `closedPnls` joins it directly (each CLOSED position → exactly one strategy). 2-reviewer adversarial PASS; 18/18 tests. **MERGED + DEPLOYED LIVE 2026-07-07** (V026 applied to `artha` — flyway head 026; strategy-signal on the H5 build, running sha == HEAD via `/actuator/info`; graduation board serves 45 strategies through the new query; both open positions backfilled `opening_signal_id`; pinned pre-migration dump taken via M30's `ay backup keep`) | [#621] |
...
- **High:** **H6** (screener reads CA-unadjusted bhavcopy), **H8** (cheat-3c mislabel — doctrine). *(DONE this pass: H9 stale-jar guard [#617], H11 Manas swing tests [#618], H12 per-book isolation tests [#619]; H5 graduation attribution BUILT + adversarial-PASS, MERGED [#621]; H4 canonical Chandelier trail — owner approved both recommendations 2026-07-07 → **MERGED + DEPLOYED LIVE** [#628] @ `c0d36310`: Chandelier is the operative live-paper Manas exit in BOTH engines, AND pyramiding disarmed (live `ARTHA_MANAS_ARORA_PYRAMID_ENABLED=false`; backtest headline flipped rs-turnover-pyramid → rs-turnover-nopyramid). Live-verified: running sha == HEAD, pyramid off in-container, both Manas strategies re-published under the new schema, swing batch enabled.)*
