...
> ✅ **Finding 1 is now CLOSED — superseded 2026-08-03.** The owner supplied a ping URL on 2026-08-02;
> `ARTHA_HEARTBEAT_SESSION_URL` is **SET** (len 56) in the container env, so the `@ConditionalOnProperty`
> bean is created and `SessionLivenessHeartbeat` is armed. The "EMPTY" measurement above was true when
> this shard was written and is now stale — ledger §0 (line ~95) records the arming. `computed` 2026-08-03.
> ⚠️ **Residual, and it is a different hole:** the sibling `SwingBatchHeartbeat` (#640, 20:15 IST) pings
> **unconditionally** — it proves the JVM was alive, NOT that the batch wrote its `swing_batch_runs`
> marker. So a stack that is UP while its batch silently fails still pings green, and the miss surfaces
> only ~12 h later via the 08:30 `SwingBatchCanary`. Owner approved making the ping conditional on the
> marker (2026-08-03); build in flight. The known way to hit it is a funnel-snapshot HTTP failure
> (`snapshotAvailable=false`, `SwingBatchRecorder:114-121`), where **exits can fire and paper positions
> can close while no marker row is written** — i.e. the canary then reports "did not run" for a session
> that partly did.
...
| **6** | **`source.optionanalytics` Upstox PCR flip — the one live freshness check never ran** | ledger §6 (deferred-by-design), "Only a LIVE market-hours freshness check remains before flipping it" | ⚠️ **WRONG — CORRECTED 2026-08-03, see note below** | ~~`sourced` — `application.yml:87`: `optionanalytics: ${ARTHA_MD_SOURCE_OPTIONANALYTICS:native}`. Still `native`; the Upstox route has never run in production~~ | Medium. Built, tested, one check from usable, invisible for a month |

> ⚠️ **Correction to finding 6, 2026-08-03.** This shard read the **YAML default** (`${…:native}`) and
> concluded the deployed value was `native`. It was not: `ARTHA_MD_SOURCE_OPTIONANALYTICS=upstox` in
> `.env:56` and in the container env, and had been for **≥28 days** — a fact already recorded at
> `docs/audits/2026-07-06-ui-data-correctness/options.md:17`, which this shard did not cross-check.
> So "the Upstox route has never run in production" was false; it had been serving the whole time
> (confirmed behaviourally: the API returned `pcr=1.7776`, byte-matching Upstox direct, while native
> gave 1.7763). **The generalisable lesson — a YAML default is not a deployed value; read `.env` or the
> container env before asserting what a flag is set to.** The comparison ran 2026-08-03 and the owner
> decided to KEEP `upstox`; ledger row **H6** carries the evidence and closes it.
