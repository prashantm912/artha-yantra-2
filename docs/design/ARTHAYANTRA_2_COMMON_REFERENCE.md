...
A7–A12 ratified 2026-06-12 from the owner feature selection (`docs/ARTHAYANTRA_2_FEATURE_PROPOSALS.md`); content tagged [FP-N]. The selection's Future items are listed in §21.1 with no design detail.
...
The proposals below were classified **Future** by the owner on 2026-06-12 from `docs/ARTHAYANTRA_2_FEATURE_PROPOSALS.md` (the same selection whose Mandatory/Recommended items became Amendments A7–A12 and the letter-suffixed phases). **A separate plan will be created for them later; no design detail for them belongs anywhere in the current plan** — this list is deliberately titles-only. Grouped under the proposal document's category headings. [owner selection 2026-06-12]
