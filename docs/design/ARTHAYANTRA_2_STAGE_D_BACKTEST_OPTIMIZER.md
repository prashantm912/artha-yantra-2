...
- `.github/workflows/ci-optimizer.yml` — ruff + pytest (**≥ 75 % line**) → image → GHCR. ⚠️ **The "**path-filtered**, gitleaks step" this line used to carry is SUPERSEDED (2026-08-04, recording [#1252](https://github.com/prashantm912/artha-yantra-2/pull/1252) @ `112e5928`, 2026-08-03).** Both halves were true as built and are false now. **(a) The workflow-level `on: … paths:` filter is GONE, deliberately** — a `paths:`-filtered workflow can NEVER be a required status check, because a path-skipped workflow's checks stay `Pending` forever and block the PR, whereas a job or step skipped by an `if:` reports `Success`. The workflow now ALWAYS triggers and a cheap `changes` job decides whether real work runs; the gated job carries `if: always()` plus a fail-closed first step, so a broken classifier reports RED rather than a skipped (= satisfied) required check. **(b) The `gitleaks` step is gone** — it duplicated the byte-identical full-history scan `ci-java.yml` already runs ungated on every PR, and `gitleaks` is itself a required context, so keeping it would have published a second check run under a required name. **(c) Consequently the job was renamed `lint-test` → `optimizer-lint-test`** (required contexts are matched by check-run NAME repo-wide, and `ci-margin.yml` had a structurally identical `lint-test`) **and PROMOTED to a required context.** `main` now requires **eight**, not six: `contracts`, `e2e`, `gitleaks`, the three `build-test` shards, `optimizer-lint-test`, `margin-lint-test`.
- `contracts/optimizer-service.openapi.json` (**FastAPI-native dump**) committed. **Gating is PARTIAL, and deliberately so** — this line used to claim "diff-gated in `ci-contracts`", which was false as written and cost 29 silently-missing paths (task_e2d15c21: the dump advertised 7 paths while the service served 36 — the whole `/api/v1/evolution/*` family, two `optimizations` paths and `/metrics`). What is actually gated:
  - **`ci-contracts`, TS-client loop** — the dump is regenerated through `openapi-typescript@7`, diffed against `contracts/gen/optimizer-service.d.ts` (warn), and the result must compile under `tsc --strict` (**hard**).
  - **`ci-contracts`, two-step chain (both halves in the REQUIRED job)** — `app routes → api-surface.json → openapi.json`. **Step A** runs `services/optimizer-service/tests/test_openapi_contract.py` against the live app, gated on an `optimizer` path classification (it needs the service's Python deps). **Step B** is `.github/scripts/optimizer_spec_staleness.py`, comparing the committed dump's operation surface to `api-surface.json`; it reads two committed files, so it is deterministic and runs unconditionally. Both **hard**. Both must be required, or the two snapshots can agree with each other while both lag the code.
  - **`ci-optimizer`, pytest** — runs the same step-A assertion, plus ruff and the full suite. ⚠️ **SUPERSEDED 2026-08-04, same change as the deliverable line above ([#1252](https://github.com/prashantm912/artha-yantra-2/pull/1252) @ `112e5928`).** This bullet used to read "It is `paths:`-filtered and **not** one of `main`'s required status checks, so it reports red rather than blocking; the contract guarantee therefore does **not** rest on it" — and then scoped making it required as future work needing "this workflow's classifier + step-gating restructure (a path-skipped required workflow stays Pending forever) and then a branch-protection change — the cleaner consolidation, owner call." **That work has LANDED.** The filter is gone, the `changes` classifier + per-step gating exist, the job is named `optimizer-lint-test`, and it IS one of `main`'s eight required contexts. **The contract guarantee now DOES rest partly on it** — a red `optimizer-lint-test` blocks the merge rather than merely reporting. The `ci-contracts` two-step chain above remains the primary gate; this is a second, independent one, so step A is now enforced by two required checks rather than one.
  - **NOT in `ci-contracts`' openapi-diff breaking gate.** Measured, not assumed: `.github/scripts/openapi_relabel_30.py` exits 2 on this document. Precisely: 3.0 *can* express nullability (`type: X, nullable: true`) and springdoc's 3.1 type-array spelling already converts — but the relabel's `anyOf` converter matches only a **bare** `{anyOf: [{$ref}, {type: null}]}`, whereas pydantic emits a `title` sibling around a **primitive** (134 occurrences, 132 with `title`). Nothing converts it, so the 3.0 validator rejects the leftover `type: "null"`. Teaching the converter that form is tractable and is the real fix; it edits the invariant guarding all four Java specs, so it needs its own fixtures (primitive `anyOf`, `$ref` `anyOf`, a genuine union that must still refuse, 3.0 validation, byte-unchanged Java output). **Until then a genuinely breaking optimizer change is caught only as a visible re-capture diff in review, never by a merge-base comparison of the document** — the one gap this list does not close. One narrow exception since #1183: `ci-contracts`' **removed-component-name gate** *does* compare this service across the merge base, but only the `components` KEY SET (a deleted or renamed schema name, which generated clients index by name). Both its sides are the committed dump — there is no capture artifact for a Python service — so it sees a rename only once the dump is re-generated. Operations, properties and types remain unguarded across the merge base.
