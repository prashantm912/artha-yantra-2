...
- `contracts/optimizer-service.openapi.json` (**FastAPI-native dump**) committed. **Gating is PARTIAL, and deliberately so** — this line used to claim "diff-gated in `ci-contracts`", which was false as written and cost 29 silently-missing paths (task_e2d15c21: the dump advertised 7 paths while the service served 36 — the whole `/api/v1/evolution/*` family, two `optimizations` paths and `/metrics`). What is actually gated:
  ...
  - **`ci-optimizer`, pytest** — runs the same step-A assertion, plus ruff and the full suite. It is `paths:`-filtered and **not** one of `main`'s required status checks, so it reports red rather than blocking; the contract guarantee therefore does **not** rest on it. Making it required would first need this workflow's classifier + step-gating restructure (a path-skipped required workflow stays Pending forever) and then a branch-protection change — the cleaner consolidation, owner call.
