...
> **Moved to the Stage-C guide.** Live mode (real Kite credentials) is now
> exercised from there — you mint a **brand-new 2.0 Kite API key/secret** after
> Stage C, so the live walkthrough belongs at that point in the timeline. See
> [docs/manual-testing-stage-c.md](manual-testing-stage-c.md) →
> *Appendix — Live mode*. The Stage-B pass above stays fully mock, zero-credential.
