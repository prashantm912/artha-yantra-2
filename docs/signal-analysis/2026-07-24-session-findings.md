...
| T12 | OI quadrant / spurt reads | — | fix the failing `/options/spurt` read; separately fix the futures-OI 1-minute capture cadence | quadrants healthy a **4th** session while cadence **improved to 211**/375 (after 198, 187, 192, 208) — cadence mechanism stays refuted, endpoint still suspect | **STRUCTURAL (defect)** | **DONE 2026-07-26 — BOTH halves closed.** `/options/spurt` was never broken (probe shape error, fine since #957). Cadence fixed + merged + deployed: [#1031](https://github.com/prashantm912/artha-yantra-2/pull/1031) (ledger G5). Root cause was **scheduler starvation, not the limiter** — both snapshot jobs shared the single default `taskScheduler` (pool 1, ~32 jobs), so while the options pass held the thread this cron could not even be INVOKED. Fixed with a dedicated `oiCaptureTaskScheduler`; the bounded limiter-retry ladder is the secondary half and only becomes reachable AFTER the isolation. ⚠️ The row's literal "dedicated quote limiter" framing was NOT built — Kite documents 1 req/s and a second limiter would put us at 2/s, over the published budget. |
...
| T9 | strategy-coverage watchdog | none | alert on emitting/published ratio drop, split by counter | 36/38 today; the 2 silent slugs are the `hero-zero` pair on a non-expiry Friday — a ratio alarm would have to tolerate this | **STRUCTURAL** | **DONE 2026-07-26 — the design REFUSED the filed one-liner, then it was built.** Design [#1024](https://github.com/prashantm912/artha-yantra-2/pull/1024), build [#1035](https://github.com/prashantm912/artha-yantra-2/pull/1035), deployed live in **DISABLED** mode (ledger G2). The emitting/published RATIO is structurally confounded — it moves with direction, window, position state and universe membership, so it cannot distinguish "nothing fired" from "nothing was watching". Replaced by a per-strategy `lastSeenPerSlug` stamp placed ABOVE the `inUniverse`/`withinSessionWindow` continues in `SignalEngine`, which measures LOAD COVERAGE rather than runtime activity. **Arming is an owner decision** — alert channel, `NOT_LIVE_RESOLVABLE` re-page cadence and OBSERVE_ONLY duration are unanswered. |
