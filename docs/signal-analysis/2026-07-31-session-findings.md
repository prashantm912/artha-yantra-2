...
| T2 | `iv_rank` dot | NULL 100% | source or drop | 13th session | STRUCTURAL | PROPOSED (carried) → ⚠️ **NOT available work — ledger row E8 is DONE** ("nothing owner-actionable today; re-open only if Sept IV-history data prompts an iv-rank-dot arming decision"), and G1 closed 2026-07-29 with T2 explicitly *carried*, not open. Per the single-status rule the ROW wins; pointer added by prose-audit shard C 2026-08-02 |
...
| T14 | margin invariant | — | sign-aware | 0 self-contradicting rows (composite-pass ∩ composite-blocked = 0) — 3rd clean | STRUCTURAL (diagnostic) | PROPOSED (carried) → ✅ **SHIPPED the next day — ledger row G17, MERGED [#1171](https://github.com/prashantm912/artha-yantra-2/pull/1171) @ b7246a3e, deployed 2026-08-01.** Honest when written; it is the newest tune table, so a fresh enumeration reads it as open — pointer added by prose-audit shard C 2026-08-02 |
