...
| boot health | ONE pre-open boot 08:17:30 IST with a **10-minute 0-loaded/38-unresolved transient** (`engine_reloads`: install 0/38/0 at 08:17:30 → install **38/0/0 at 08:27:15**; boot line `loaded 38 published (0 dropped, 0 failed)` at 08:27:15 in live logs). Same class as 08-14's transient but ~90 s there vs **~10 min** today — watch the growth. Resolved 48 min before open; no session impact ⚠️ **"WATCH THE GROWTH" IS WITHDRAWN 2026-08-17 ~18:40 — there is no growth, and the cause is fully known.** `computed` over every `loaded=0/unresolved=38` transient in `engine_reloads`: **08-06 132s · 08-07 74s · 08-08 123s · 08-10 72s · 08-12 72s · 08-13 127s · 08-14 71s · 08-17 585s.** That is not a trend, it is one outlier at **4.4× the previous maximum**, and it is a STEP not a slope. **Cause: the Kite access token was DEAD at boot.** The stack came up 08:17:30 into an expired session (`kite session status -> TOKEN_EXPIRED`; the owner's login was refused all morning with `Invalid api_key` until the Kite Connect app was re-enabled), so `FuturesUniverseResolver` returned `503 DATA_STALE` for all 38 universes and NONE could resolve. The owner logged in at **08:26:38**; `SignalEngine` logged `kite.status CONNECTED received — requesting bounded strategy reload` and installed 38/0/0 at **08:27:15** — 585 s after boot, and **37 s after the credential came back**. The transient lasted precisely as long as the token was dead. **Nothing is degrading; do not open an item for this.** The genuine lesson is the opposite of a growth watch: this metric is a faithful proxy for CREDENTIAL health at boot, not for engine health, so a long transient should send you to the Kite session before anything else |
...
| NEW-5 (08-17) | this routine's own `--admin` permission | ⛔ **RAISED AND DECLINED BY THE OWNER, 2026-08-17 ~18:45 — do NOT re-raise it** | The scheduled task's own instructions carry *"squash admin-merge is allowed for docs-only PRs"* (`~/.claude/scheduled-tasks/post-market-session-analysis/SKILL.md:24`). That permission is **stale on its face**: `--admin` bypasses **all nine** of main's required contexts, and `lock_branch` — the only reason it was ever needed — was set false on 2026-07-26, after which CLAUDE.md has read *"merge normally; reaching for `--admin` means something is actually wrong"*. Flagged because the job runs **daily and unattended**, so the permission is standing rather than case-by-case. ⚠️ **No harm has occurred and that is measured, not assumed: today's #1396 merged with 15 SUCCESS / 3 SKIPPED / 0 FAILURE** — the permission was never exercised against a red check. **Owner ruled: leave it. Docs-only PRs are low risk and an unattended routine should not be blocked by an unrelated red shard.** Recorded here so the next enumeration pass finds a DECIDED item rather than re-discovering a stale-looking permission and spending a round-trip on it |
