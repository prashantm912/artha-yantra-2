...
Refreshed **2026-07-27** (through the 2026-07-27 session). Note the challenger roster changed: the
`composite-070` book was replaced by `composite-055`, so its all-time row is retired.

⚠ **2026-07-28 added NOTHING to this table — no book took a single row.** The monthly-expiry composite cap
(0.5479) sat below the 0.600 threshold all session, so the champion book had no composite-passing rejection
to open on and the challengers had no eligibility set. Every number below is therefore still "through
2026-07-27", and `composite-055`'s 11 closes have now sat out **two** consecutive sessions.

| variant | closed | net wins | total net ₹ | total pts | verdict-so-far |
|---|---|---|---|---|---|
| champion | **219** | **88** | **−35,153.63** | −246.5 | **best session on record on 07-27 (+₹9,007.08 over 41 closes)** took the book −44,161 → −35,154. Deduped it is 9W/12L over 21 events, with two clusters (11:27 NIFTY +₹7,707, 09:48 SENSEX +₹5,395) carrying the spread. Reading unchanged in shape: the gate **rejects losers on chop/flat/quiet days and winners on trend days** — 07-27 was a trend day |
| composite-055 | 11 | 3 | **−1,542.15** | −10.2 | **took ZERO rows on 07-27 and correctly so** — the eligibility set (composite in [0.55, 0.600) with no other failed rail) was empty. Its 11 closes are now getting staler rather than more decisive, and they all predate #991, which changed what the composite means |
| vol-12k5 | **38** | **12** | **−9,331.31** | −218.3 | **first winning session (+₹4,059 over 10 closes on 07-27)**. Still the **least-bad of the two volume books** — the `vol-12k5 > vol-off` ordering has survived every session where both traded |
| vol-off | **48** | **14** | **−17,014.10** | −388.9 | also won on 07-27 (+₹2,317 over 12 closes) but took MORE entries for LESS money than `vol-12k5`, which is the ordering in miniature |
