...
- **⚠️⚠️ T29 — THE EXIT MODEL, NOT THE ENTRY GATE, WAS THE DOMINANT P&L TERM ON 2026-07-29 (NEW).** Three
  ...
  −538.50 pts, with ZERO take-profit and ZERO stop touches**. **Classification: UNDETERMINED, pending a
  ...
  not. Do not act on the trend day alone. Coordinate with the exit-band runbook.
- **⚠ T27 — the relative volume floor's SHAPE is wrong even though its LEVEL is vindicated (NEW,
  STRUCTURAL).** `1.5 × MEDIAN(prior N)` is robust to one outlier, but the opening surge is not one bar:
...
- **⚠ T28 — a THIRD dot state exists: frozen (NEW, STRUCTURAL).** `macro.atmIv` carries exactly one
...
- **✅ T3 CLOSED AS A KNOB — the ten-session-old ground-truth query finally ran and killed it.** The
  ...
  lifts the cap 0.9574 → 1.0000). ⚠ 07-28's "first life ever, 1.7%" was an expiry-day artefact and was
