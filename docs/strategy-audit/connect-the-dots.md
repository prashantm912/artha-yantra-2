...
| RSI multi-timeframe cross-check: RSI(5m)<75/80 & RSI(Daily)<75 for longs (mirror >25/20, >25 shorts) | 3.10 Filters / 6.10 timeframe | NONE | Scorer reads RSI only on the **3m primary** (`ConnectTheDotsScorer.java:78` `c.rsi14()`; YAML `rsi14` alias `timeframe: 3m`); no 5m or Daily RSI series is declared or scored | The doc's "RSI cross-checks 5-min and Daily" (line 2621) and the §3.10 Filters clause "RSI(5m) below 75/80 and RSI(Daily) below 75 for longs" (line 1124) are NOT automated — only the 3m RSI band gates. Manually confirm the 5m and Daily RSI are not overbought (longs) / oversold (shorts) before taking. Automatable: true (add 5m/Daily RSI dots) |
...
| Global cues — DOW 30 futures must align | 3.10 Setup 3 / Entry / Filters | PARTIAL | Dow rides `ConnectingDotsService.dowFactor` (file:316-333, OI-page UI only); the **scalper scorer `Macro` has NO dow field** — not consulted by `ConnectTheDotsScorer` | Dow is shown on the OI matrix page but does NOT gate the scalp signal. Manually check DOW futures direction. Covered as `global_cues_ok` manual check (`ScalperManualChecks.java:51-55`, §4.7). Automatable: partial (Dow LTP exists via GlobalQuoteSource; would need wiring into the scalper Macro) |
| Global cues — Dollar index (>105 neg / <90 ideal), Asian, European, Crude, Bond yields, USD/INR | doc-line 406 (§3.1 Filters), referenced by §3.10 Filters | NONE | No field in `ScalperGateContext.Macro` (file:59-68); not in any gate/scorer | None of dollar-index / Asian / European / crude / bonds / USD-INR are automated anywhere. Manually scan global cues. Covered (coarsely) by `global_cues_ok` manual check (§4.7). Automatable: false (no live feeds for these in the platform). **[v2 doc-§ fix]** the ">105 / <90" thresholds are real numbers but live at doc line 406 (§3.1 Filters), not §3.10 itself — §3.10 only cross-references them. |
...
| Support-trade / pullback entry: enter only at the Supertrend level on a pullback after an upside crossover; take pull-back entries near VWMA/ST/VWAP (don't chase) | 3.10 Exit-Scaling / Edge cases | NONE | YAML entry gate is `close > vwap` only (`*.yaml` `entry_rules.gate.all`); no pullback/retrace-to-ST/VWAP entry trigger in `ScalperConfluenceGate` or `ConnectTheDotsScorer` | The doc's defining discipline — wait for a retrace to ST/VWAP/VWMA rather than entering on the breakout extension — is not modelled; the seam fires on any bar where `close > vwap` and the confluence holds. Manually wait for the pullback to ST/VWAP support before entering. The "net-cautious → enter only on a retrace to ST/VWAP, do not chase up-moves" S21(b) refinement is likewise manual. Automatable: true (a pullback-proximity entry condition is feasible) |
...
| "One good trade" clean-setup discipline | 3.10 philosophy (maps to §3.1 checklist item) | MANUAL_COVERED | `ScalperManualChecks.java:56-60` `clean_setup` (§3.1) | Manual discipline check. Automatable: false. **[v2 doc-§ note]** the "one good trade" heading cites §3.10 but the checklist item is filed under §3.1 (per README quality-flag) — same discipline, looser pointer. |
| Instrument = BUY CE/PE only; "Sell PE / Sell CE" are directional synonyms (not naked selling) | 3.10 Execution Notes / 6.10 uncertain | FULL (matches buy-side reading) | `StrikePicker.pick(...)` and `StraddleLegPicker` only ever return BUY legs; `ScalperConfluenceGate.java:130-131` documents short-straddle (SELL) is SPAN-deferred | The doc itself flags this as UNCERTAIN (line 1132): the matrix "Sell PE/Sell CE" labels read as directional synonyms for the buy-side trade, resolved in S22 to "framework buys CE/PE (or futures)". The automation faithfully implements the buy-side interpretation — no naked option selling. No manual action; noted to record the automation matches the doc's resolved (buy-side) reading. |
...
Independent second-pass review against the live automation (`scalp-connect-the-dots-*.yaml`,
`ScalperConfluenceGate`, `ConnectTheDotsScorer`, `ScalperGates`, `ScalperConfig`, `ScalperOiProps`,
`StrikePicker`, `ScalperManualChecks`, market-data `ConnectingDotsService`). v1 was largely accurate and
thorough; the changes below are additive plus a few precision fixes.

**Missed rules added (3 new rows):**
- **RSI multi-timeframe cross-check (NONE).** Doc 3.10 Filters (line 1124) and 6.10 timeframe (line 2621)
  require "RSI(5m) below 75/80 and RSI(Daily) below 75 for longs" (mirror for shorts). The scorer reads
  RSI only on the 3m primary (`ConnectTheDotsScorer.java:78`; YAML `rsi14` alias `timeframe: 3m`) — the
  5m/Daily cross-check is entirely unautomated. v1 had no row for it.
- **Support-trade / pullback entry (NONE).** Doc 3.10 Exit-Scaling + Edge cases make the defining
  discipline "enter only at the Supertrend level on a pullback after an upside crossover; take pull-back
  entries near VWMA/ST/VWAP, do not chase." The YAML entry gate is `close > vwap` only — no pullback/
  retrace trigger. v1 mentioned "wait for pullback" only inside the `not_parabolic` manual-check row; it
  is its own framework-native entry rule and now has its own row.
- **Instrument = BUY CE/PE only; "Sell PE/Sell CE" are directional synonyms (FULL, matches doc).** The
  doc's own UNCERTAIN item (line 1132). The automation only ever returns BUY legs
  (`StrikePicker`/`StraddleLegPicker`; short-straddle SELL is SPAN-deferred, `ScalperConfluenceGate.java:130-131`),
  so it faithfully implements the S22-resolved buy-side reading. v1 had no row recording this.

**Inaccuracies corrected (doc-§ pointer fixes, no status changes):**
- **Dollar-index/global-cues row** — v1 attributed ">105 neg / <90 ideal" to "§3.10 Setup 3 / Filters".
  Per the README quality-flag, those numbers live at doc line 406 (§3.1 Filters); §3.10
  only cross-references them. Doc-§ corrected; status stays NONE (genuinely unautomated).
- **"One good trade" row** — heading cites §3.10 but the checklist item is filed under §3.1 (README flag);
  noted in-cell. Status stays MANUAL_COVERED.
- **Manual-check line citations** — corrected three off-by-one drifts: `vix_normal` (the §4.5 item spans
  `ScalperManualChecks.java:46-50`, not :48), `global_cues_ok` (`:51-55`, not :51), `clean_setup`
  (`:56-60` reaffirmed). Cosmetic; no status change.

**Confirmed accurate (spot-checked against source):** the 09:45 / 11:00–13:00 / 15:30 time rails
(`ScalperGates.java:22-25`), VWAP hard-gate + weight (`ConnectTheDotsScorer.java:32,71,114-115`), the
RSI §4.2-vs-§3.10 band override (`ScalperGates.java:76-84`), the OI-props thresholds (cross 50 /
drastic 50000 / iv-gap 0.10 / iv-both-high 0.40 / spurt 50/50, `ScalperOiProps.java:32-43`), the
live-no-op VIX/Dow/dollar gaps (`MarketOiClient.java:396-397`; Macro has no dow/dollar field), the
60m-bias hard-gate (`:111,115`), FII fetched-but-unscored (`MarketOiClient.java:375-383`), and the
