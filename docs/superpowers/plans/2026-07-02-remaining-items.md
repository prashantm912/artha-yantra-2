...
| A13 | `eval-stall-canary-fix` | ⚠ URGENT (with B1) — 3rd silent eval stall 2026-07-10 14:52→close and the #634 `SubscriberHealthCanary` logged NOTHING + did not re-subscribe (in-image, built 08:46). Verify/fix: does it page + recover on a GREEN-feed-no-bar gap? Both 07-07/07-10 stalls sat in the last ~40 min — its window/market-hours guard likely misses them. Missed-EXIT risk, same class as B1 | rollup watchlist; 07-10 findings §6; §1 `signal-eval-redis-subscriber-watchdog` | HOLD | TODO |
