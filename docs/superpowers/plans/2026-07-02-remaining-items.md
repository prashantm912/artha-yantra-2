...
| task_cf5d58c8 | 2026-07-13 | backtest exit_reason CASING divergence surfaced by P1-3 (#799): candle path persisted lowercase while options/deep-swing/counterfactual/live-paper use UPPERCASE. | **BUILT-HOLD [#801](https://github.com/prashantm912/artha-yantra-2/pull/801)** — owner-confirmed **UPPERCASE canonical + backfill** (evidence flipped the chip's lowercase guess: live paper close_reason is UPPERCASE, so is the 34k-row majority; only the candle path changes). One `ExitReasons.canonical()` helper at the single `TradeRepository.insertAll` chokepoint (idempotent for the already-upper paths) + V018 backfill of the 698 lowercase rows + case-insensitive FE grouping. 3-reviewer pass 0 defects (migration SAFE / chokepoint complete / premise holds). **PR OPEN for owner diff review** — HOLD (owner-facing value + data backfill). Caveats: reconciliation metric still DORMANT (forward-readiness, not a live fix); casing-only leaves a paper-vs-backtest vocabulary gap; V018 is a one-way door (harmless — casing non-load-bearing) |
