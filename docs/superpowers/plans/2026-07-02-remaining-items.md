...
| `upstox-margin-route` | **F9 risk layer via Upstox margin API** — route live paper margin sizing through `POST /v2/charges/margin` instead of the local `.spn` appliance. **VERIFIED LIVE 2026-07-04:** the login-free analytics token (`deploy/secrets/upstox_analytics_token`) authorizes it — a 1-lot short returned real `span_margin`/`exposure_margin`/`total_margin`/`required_margin`/`final_margin` server-side, NO `.spn` file. `GET /v2/user/get-funds-and-margin` = live available capital (down 00:00–05:30 IST → 423); `GET /v2/charges/brokerage` = pre-trade charges (cross-checks F8 `FeeConstants`). Build: an Upstox-margin `SpanEngine` impl for live sizing (marginism appliance #126 = offline/backtest fallback); `advised_lots` + margin snapshot on paper entries; heat cap + daily-loss governor advisory-first behind a flag. **Gotcha:** `quantity` must be a multiple of the contract lot size (UDAPI1104 otherwise) — read lot size from the Upstox instrument master; ≤20 legs/basket. | roadmap F9; ADR-0002 Upstox side-channel | **BUILDABLE now** — no owner gate, no `.spn`. Supersedes `span-real-spn-broker-parity` (§2). |
