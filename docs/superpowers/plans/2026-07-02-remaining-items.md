...
| B5 | `fid-p0-5-btst-exits` | BTST exits never simulated (backtest branch never opens a position) — EVO runs BTST SIM_BLOCKED until fixed | FID §10 P0-5 | HOLD | TODO |
...
| B7 | `fid-t3-actor-plumb` | `created_by` actor plumbed through job/run/version writes | FID §5 T3; EVO §13 row 4 | clean | TODO |
...
| B11 | `fid-p1-9-context-lookahead` | Gate context advancement on bucket END ≤ bar time — 1d-context strategies see today's close intraday; SIM_FIRST sims inherit the artifact | FID §10 P1-9, §12 #14 | HOLD | TODO |
| B12 | `fid-p1-10-intrabar-touch` | Wire `IntrabarExitResolver` as opt-in `touch_basis: bar_hl_worstof` + per-run basis provenance | FID §10 P1-10 | HOLD | TODO |
| B13 | `fid-p1-11-expiry-settlement` | Settle backtest legs held past expiry at intrinsic + exercise STT (`exit_reason='expiry_settlement'`) | FID §10 P1-11 | HOLD | TODO |
...
| E7 | `f2-proposals-pass` | 10x F2 dot/gate proposals pass — self-triggers at ≥5 rollup sessions (4 logged; the 5th logs at today's 15:53 post-market run → run F2 after it); carry the 07-07 rollup owner-Qs (straddle-path threshold, 0DTE-straddle book routing, SENSEX scalpers still fixed-125k). Cadence VERIFIED 2026-07-10: 07-08/07-09 files missing because the stack was DOWN (owner-confirmed 07-08 down through market hours; the task fired both days and skipped correctly — no task defect; 07-08 = a market-hours capture hole, Upstox-backfillable if ever needed) | 10x F2; `docs/signal-analysis/rollup.md` | TODO |
...
| F9 | `shadow-evidence-hygiene` | (a) One-shot backfill of `pnl_net` for the 20 pre-F8 2026-07-03 closes (entry/exit LTPs + cost model exist — makes net honest across all history; live-DB write, shadow-only table); (b) owner pick on `composite-070`: drop it, OR re-point to a LOOSENING floor (e.g. `composite-055`), OR wait for the accepted-entry shadow extension (INT-design HOLD). Month-end E1 must use points or post-07-04 net until (a) lands | F4 findings 2026-07-10 | (a) clean / (b) owner | TODO |
