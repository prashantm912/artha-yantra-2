...
Wave 2 in flight (receipts pending): A10 prune job, F2 Manas N+1 batch reads, F3 FIFO probe, INT-I1 FE feed/Focus.
INT next = I2 (context breadth); C4 fired-rail side-channel still TODO; EVO next = E4 (proposals/promotion pipeline).
...
| A10 | `p1-snapshot-retention` | Schedule `prune_options_snapshots` (horizon = owner pick) | APP §3.3 | owner | **OWNER-DECIDED 2026-07-12: 365-day horizon** — build IN-PROGRESS (wave-2 builder; chunk-wise Timescale retention, never row-DELETE) |
