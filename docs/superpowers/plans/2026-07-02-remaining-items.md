...
| task_cf5d58c8 | 2026-07-13 | backtest exit_reason CASING divergence surfaced by P1-3 (#799): candle path persists lowercase (stop_loss…) per the DoD while options/deep-swing/counterfactual/swing use UPPERCASE (STOP_LOSS…). Cosmetic today (FE renders raw, consumers case-agnostic); blocks the optimizer's dormant exitReasonDivergence metric until normalized. HOLD-tier (changes other paths' owner-facing values). Pick one casing + normalize + case-insensitive FE grouping. | OPEN |
