...
| MV-7.3 | **Staggered stops** (§2.2) — partial closes at tiered stops (e.g. 4% half + 8% half ≈ 6%); a staggered-stop executor (multi-leg partial close, not single-precedence full close) | EXTEND `ExitEvaluator` | MV-7.2 | **SPEC (PR-P)** | Parity-safe design recon'd → `2026-07-04-minervini-partial-close-build-spec.md` (6–7 files; side-channel `qtyFraction`; `applyExit`; fraction-aware `ReplayEngine`; new `golden-minervini-scaled/` fixtures; `GoldenDeterminism`+`BacktestParity` byte-identical gate). Deliberately a supervised pass (parity-firewall + reliability bar not yet met). | build-spec |
| MV-7.4 | **Sell-into-strength / scaled profit exit** (§2.6/§3.6) — new exit rule type (tiers `[{profit_pct, qty_pct}]`), free-roll (sell half at 2–3R), 50-day-MA trailing stop | EXTEND schema + `ExitEvaluator` | MV-7.2 | **TRAIL DONE (PR-Q); scaled SPEC (PR-P)** | **50-day-MA trail DONE:** `minervini-vcp-trail.yaml` (`trailing_stop {basis: indicator, alias: sma50}` — existing engine type, no parity surface); `SwingTrailTest` exits on a close below the 50-day MA above the 8% stop; goldens byte-identical. **Scaled tiers** = the partial-close executor (PR-P spec, MV-7.3 row). | `minervini-vcp-trail.yaml`, `SwingTrailTest.java` |
