...
- **No new trade automation.** The only automated ENTRY flow remains the existing
  per-book `auto_paper_trade` (shipped enabled for the 3 books, owner-editable via risk
  settings, P1-§2.2); existing automated EXITS (15 s brackets, expiry settlement, engine
  swing exits) also remain untouched. Everything this layer adds is display,
  propose-with-one-click-approve, or draft.
...
2. **Insight plane — strategy-signal-service, new Modulith module `insights`.** Owns
   ...
   calls market-data over REST for gate context (the `scalper/MarketOiClient` in-code
   precedent). Modulith rule respected the way `notifier` consumes signals events without
   `signals` ever importing notifier: `insights` listens to `signals`/`paper` events, and
   those modules must never import `insights` (adds no cycle — verified against the
   existing module import graph; `ModularityTest` covers the new package automatically).
...
|---|---|---|
| **AUTO** (no approval; display/derive only) | computed and shown; reversible by ignoring | insight generation, priority scores, digests, badges, dedupe/suppression, journal-entry DRAFTS with links prefilled, insight follow-flags (an insight-scope "watch" marker — distinct from marketdata watchlists), freshness/trust chips, EOD summary |
| **PROPOSE** (one-click approve; every action calls an existing endpoint) | nothing executes without a click; the audit chain = the `insight_actions` row (§2.4) plus the target endpoint's own trail where one exists (signal status flips have no table audit today — there the insight_actions row IS the trail) | take-signal prefill (calls `POST /signals/{id}/taken`), ticket prefill with book+qty (`POST /paper/orders`), watchlist add, journal draft accept (`POST/PUT /journal`), dismiss-stale-signals batch (`POST /signals/{id}/dismiss` per item), acknowledge sell-decision (endpoint = §13 row 8's ADD) |
...
|---|---|
| Signals list is time-ordered; every published strategy can fire into it; no triage (P1-§2.1) | Priority-ordered Focus queue with A–D bands + per-component explanation |
...
| Notifications fire-and-forget to ntfy/Telegram; no in-app queue (P1-§6.1) | Insight feed = the in-app notification center P1-§6.1 asked for, with severity floors and suppression |
...
New table `strategy.insights` (next strategy-lineage migration — V027 at time of writing;
insert-mostly: status/priority mutate in place and §2.5 prunes suppressed-INFO rows, so
UNLIKE the strictly append-only `signal_rejections`; per-table `GRANT` to `ay_strategy`
follows the lineage convention):
...
CREATE INDEX idx_insights_open ON strategy.insights (status, severity, generated_at DESC);
...
| Type | Trigger | Inputs (existing per P1-§11, or a §13-listed ADD where marked) | Example title |
|---|---|---|---|
...
| `REJECTION_NEARMISS` | 15-min sweep over `signal_rejections` | `blocking_margin` (API `blockingMargin`) + `blocking_rail` + rail-counts | "3 rejections within 5% of firing on volume-floor since 10:00" |
| `REJECTION_RAIL_TREND` | EOD sweep | rail-counts windowed rollups (the endpoint takes arbitrary from/to; the EOD sweep accrues per-day values into insight evidence) | "supertrend-alignment blocked 41% today vs 12% 5-day avg" |
...
| `MARKET_STRUCTURE` | digest, coarser | breadth thrust flags, sector rotation, vix band change | "Breadth thrust: A/D 4.1, 87% above 20-DMA — strongest in 12 months" |
| `RISK_HEAT` | on `paper.events` + 5-min sweep | in-module heat read (§3.2 risk row), advised lots vs actual, concentration | "Scalper book heat 78% of cap after this fill" |
| `RISK_STALE_TICK` | bracket-starvation counter (P1-§8 V3 — an ADD; interim fallback trigger = `/market/health/data` per-instrument tick freshness) | tick freshness | "SL/TP unevaluated 4 min on SENSEX 81200 PE — no ticks" |
...
| `STRATEGY_EVIDENCE` | 21:10 nightly (after the 21:00 graduation eval) | graduation rows, `strategy_graduations`, sell_decisions (ADD, §13 row 8) | "manas-arora-breakout crossed PF 1.3 (20 trades) — TAKE_ELIGIBLE" |
...
2. **Scheduled sweeps** — `@Scheduled` IST crons alongside the existing scheduler
   inventory (P1-§1.2): 15-min intraday sweep + 5-min risk sweep (market hours), 16:05
   EOD digest (after the three 15:45 jobs settle; candle-derived fields fail-soft
   DEGRADED if the 15:45 EOD candle backfill is still running), 21:10 post-batch
   strategy-evidence sweep (after the 20:00/20:05 swing batches AND the 21:00 graduation
   evaluator — `GraduationPromotionScheduler` cron `0 0 21`).
...
3. **Severity floors per channel**: in-app feed shows all; WS toast ≥ NOTICE; ntfy ≥ WARN;
   Telegram ≥ CRITICAL. Delivery adapter caveat: the existing per-strategy opt-ins +
   `notification_events` audit (P1-§11.20) cover strategy-scoped insights only —
   `notification_events.strategy_id` is NOT NULL today, so market/dataops-scoped insight
   delivery needs a small migration (nullable `strategy_id` + `insight_id` column) and its
   own channel config under `artha.insights.delivery.*` (§13 row 20).
4. **User mutes**: per-type/per-scope mute stored in `user_prefs` (P1-§11.18); a
   `MUTE_TYPE` action creates it.
5. **Digest batching**: INFO-severity insights never push — they collect into the 16:05
   EOD digest insight. Band→severity mapping for pushes: A-band `SIGNAL_PRIORITY` emits
   at WARN, B at NOTICE, C/D at INFO (so the default floors push A to ntfy, A+B to WS
   toasts, and C/D stay feed/digest-only).
...
|---|---|---|
| Edge margin | 0.30 | (composite − threshold) from the signal's own frozen score_breakdown, normalized into the strategy family's configured band (`artha.insights.priority.edge-band.<family>`, e.g. scalper 0→0.10 maps 0→1, clamped). NOT a re-score — a normalization of the strategy's own margin. |
...
| Risk headroom | 0.15 | 1 − heat%/cap, read in-module from `RiskService` (the `/paper/margin-heat` endpoint returns absolute margins only — no heat%/cap fields); −0.25 penalty if an open position already holds the same underlying+side (concentration); pre-trade advisory lots (recomputed via the pure `PaperService.advisedLots` static — `advised_lots` exists only on OPENED positions) == 0 → component 0. Fallbacks: cap unset/disabled → concentration+lots terms only; unpriced/equity books (margin null) → neutral 0.5 with an evidence note. |
...
Every component is `clamp(0,1)` AFTER all bonuses/penalties — the concentration penalty
and out-of-band margins cannot push a component negative or above 1.
...
```json
priority_detail: {
  "score": 82.75, "band": "A", "trustCap": 1.0,
  "components": [
    {"key":"edge","weight":0.30,"c":0.90,"points":27.0,
...
option_ltp × the signal's `suggested_qty` — already lot-aligned UNITS, so no extra ×lot) ·
margin estimate (existing `POST /market/margin` for
...
- **Near-miss rank**: `closeness = |blockingMargin| / threshold` per rejection, computed
  ONLY for numeric rails with non-null operand/threshold and threshold ≠ 0, with per-rail
  direction normalization (upper-bound rails invert sign); enum/time-window rails are
  listed separately, never ratio-ranked. The `REJECTION_NEARMISS` insight lists the top-N
  closest-to-firing per session with their single blocking rail — "what almost fired" is
  the owner's cheapest tuning signal (precedent: the 2026-07-02 volume-floor forensics,
  done by hand then, automated here).
...
- **Fired-vs-rejected contrast** (P1-§6.6), in two stages because of a data asymmetry:
  FIRED signals persist NO confluence-rail operands today (`scalper_detail` carries dots +
  aggregate; `score_breakdown.gate.operands` covers only the YAML chart-gate — rejections
  are the only rows with per-rail operands). Stage 1 (buildable now): composite score +
  dot-supports contrast between fired and rejected rows for a day + strategy. Stage 2
  (full per-rail contrast): needs a fired-side rail-operand side-channel on emission —
  parity-safe additive per the golden side-channel rules; §13 row 19.
...
No new bars, no new scoring — the graduation thresholds (20/1.3/>0/≤25%, the
`GraduationService` defaults) and the stricter GRADUATED bar (≥50, Sharpe ≥0.5,
`GraduationPromotionService`; F7 #577) stay canonical, and strategy-version ranking stays
the evolution engine's. This layer makes the EVIDENCE legible:
...
1. **Qualification dossier** — `GET /api/v1/insights/strategy-dossier/{strategyId}`:
   ...
   summary if scalper (existing shadow-summary read + a small per-strategy filter param —
   the `strategy_slug` column exists, the endpoint just doesn't filter on it yet), latest
   backtest refs (existing
   ...
   pages.
2. **Threshold-crossing insights** — the 21:10 sweep ALWAYS writes one suppressed-INFO
   snapshot row per strategy (exempt from the 90-day prune) and compares today's board vs
   yesterday's snapshot row (persisted in the insight evidence itself; no new table —
   the always-write guarantees "yesterday" exists even on no-change nights): crossing
...
4. **Evolution-engine seam**: when the evolution engine ships, its owner-gated proposals
   ...
   coupling now beyond reserving the scope string `strategy:<id>`. Two seams for its
   E-phase integration to close: (a) until then the evolution engine pushes its own ntfy
   directly — two independent push producers, outside this layer's floors/suppression;
   (b) mirrored proposal insights must expire with the proposal's 7-day expiry so
   deep-links never dangle.
...
1. `GET /api/v1/market/context/options-digest?name&expiry` — from `options_chain_snapshots`
   ...
   migration (top-5 set now vs open — set diff); ATM straddle premium now vs open (the
   premium fold, window-anchored to 09:15 — existing fold, new window param); ATM IV vs
   its percentile over `iv_daily_summary` history (the existing `IvAnalyticsService`
   already enforces a 60-day floor with an `insufficient` flag — ~18 sessions accrued
   today, so this field reports insufficient for ~2 more months); 4-state OI structure
   verdict (existing `OiInterpretation` aggregation).
2. `GET /api/v1/market/context/futures-digest` — per captured underlying (indices + 17
   ...
   existing server-side spurt classification), basis vs spot, term-structure state
   (existing `/term-structure`), banks summary (existing banks fold). Basis/spot ride
   LIVE quote reads (not snapshot folds) — these fields fail-soft DEGRADED off-hours,
   never 5xx (the `/term-structure` stale-fallback precedent).
3. `GET /api/v1/market/context/equity-digest?date` — from bhavcopy tables: A/D ratio
   (existing breadth fold) + above-20/50-DMA counts (a NEW universe-wide windowed fold
   over bhavcopy closes — no above-MA fold exists today) + day-over-day; breadth-thrust
   flag (10-day A/D moving-average crossing 0.618, standard Zweig definition,
   config-pinned); top/bottom-3 sectors day-over-day rotation (needs a date parameter on
   the sector fold — today it hardwires latest-session); delivery% z-score outliers vs
   each symbol's trailing 20 sessions (a NEW universe-wide window SQL — the existing fold
   is per-symbol); returns summary (5-window winners/losers off the existing
   `/equity/returns` fold); index-contribution concentration (top-5 names' share of
   index move).
...
5. `GET /api/v1/market/context/day-context` — the dashboard one-call: bundles 1–4's
   headline fields + VIX level/Δ/band (live quote — fail-soft off-hours like basis/spot)
   + index price action (gap-open %, day range vs 20-session average range, VWAP side,
   day structure trend/range — from existing candle reads) + overnight global cues
   (top-mover world indices off the existing `/market/world-indices` read, fail-soft when
   analytics off) + session phase + holiday proximity (`MarketCalendar`, already on
   market-data's classpath via `IvRollupJob`) + ingest-trust summary (§7). This is what
   the Focus panel and the day-context strip render.
...
index price-action edges (gap-open ≥ config %, day range ≥ k× 20-session average range,
VWAP-side flip — all from existing candle reads).
Each threshold and its baseline (open / prior bucket / prior EOD — the three baseline
conventions the existing folds use: spurt's session-open window, strike-series bucket-lag,
and trending's `baseline=peod`) is named in the insight evidence.
...
(page-level row pre-filter — DataTable ships per-column filters only, the global box is
the page's); saved views via `user_prefs`; bulk ack/dismiss (P1-§4.2.7 bulk-action gap);
suppressed toggle ("show what was muted and why"); 90-day default window with day picker
over unbounded retention (the ONLY deletion is §2.5's suppressed-INFO 90-day prune —
older non-suppressed rows stay queryable via the picker). Row click → the same explain
drawer everywhere.
...
signal/position ids — the P1-§2.2 write-only-chain fix made usable); **charts**: every
evidence chip with a symbol deep-links to `/charts?symbol&interval` (signal/trade markers
already render there per P1-§2.3), and signal markers gain the priority band in their
tooltip — the insight→chart hop is one click; **risk calculator** gains a "load from
signal" prefill (leg, entry, SL from a signal id) and the P1-flagged `POST /market/margin`
tie-in rides the same change; **multiple window** whitelist extends by two panes (Focus
panel, insight feed) so the workspace can keep triage visible; **strategy editor** gains
an evidence side-panel (the strategy's rail-trend + near-miss insights) so gate edits are
made against observed rejection behavior; **orders page** gets the trust chip only
(broker read-through otherwise untouched); cmdk palette registers "insights for
<symbol>", "compare selected", "open dossier <strategy>" — WHEN P1-Phase-4 activates the
palette (dead scaffold today); until then these are menu entries + row actions.
...
Market-data reads via a `ContextClient` (RestClient, `MarketOiClient` structural
precedent; timeouts follow `PaperMarginClient`'s explicit factory — 1.5 s connect / 2 s
read, since `MarketOiClient` itself sets none; failures → that family DEGRADED, never an
...
hundreds/day (12 types, dedupe, single owner) — the same order of magnitude as
`signal_rejections` (~440–640 rows/day observed), i.e. operationally trivial.
...
Weekly quality report (EOD job → a NOTICE insight whose evidence payload IS the report),
per type and band:
...
- **Explainability**: evidence click-through rate per type + share of insights whose
  every explanation clause has a matching evidence entry (a template-lint check, not a
  human metric).
...
1. `SignalEmitted` → priority generator: edge 0.90 (composite 0.81 vs 0.72 = margin 0.09
   on the scalper 0→0.10 band → 27.0 pts), context 0.75 (6/8 dots; options-digest PCR
   0.87→1.14 since open agrees → 15.0), track record 0.68 (PF 1.9, 34 trades,
   TAKE_ELIGIBLE → 13.6), risk 0.81 (heat 34%, no NIFTY exposure → 12.15), freshness 1.0
   (12 s old → 15.0). Trust OK (ticks live, dots alive, capture 40 s fresh).
   **Priority 82.75 (A)** — the sum reproduces under §3.2's own formula.
...
4. Take (PROPOSE): ticket opens prefilled — instrument = tradeable leg, qty = the
   signal's `suggested_qty` (already lot-aligned; the advisory-lots recompute renders
   beside it as a chip), book = scalper (selector visible per P1-Phase-2), SL/TP from
   the signal.
...
6. 10:32 exit at TP. `paper.events CLOSED` → journal draft updated with outcome tag;
   ...
   `expires_at` set.
7. 16:05 EOD digest insight: "1 A-band taken (+₹2,340), 2 B dismissed, 4 C/D expired
...
generators (clean — display-only) · `notification_events` read endpoint + the
nullable-`strategy_id`/`insight_id` migration (§13 row 20) (clean) · `/insights` feed
...
(interim health/data trigger; upgrades to the V3 counter when P1-Phase-1 lands) (clean) ·
...
| 2 | Rejection schema (blocking_margin/`blockingMargin`, blocking_rail, diagnostic, rail-counts, shadow-summary, dot-health) | §11.2 | EXISTS | AS-IS | — |
| 3 | Paper/positions/margin-heat + graduation board (§11.4/9) + shadow endpoints (§11.2/§11.19) | §11.4/9, §11.2/19 | EXISTS | AS-IS (dossier's per-strategy shadow slice = a small filter param on the existing summary — `strategy_slug` column exists) | — |
| 4 | Analytics folds this layer's digests re-use (chain/spurt/trending/premium/active-strikes; futures; breadth/sector/delivery/contribution; FII reads) | §11.12–15 | EXISTS | AS-IS, three exceptions | digests are new endpoints over these folds — no new raw data; the three §6.1.3 exceptions (above-MA counts, universe delivery-z, sector date-param) are NEW folds over the same existing tables, built in I2 |
...
| 6 | **Risk-governor enforcement on manual orders (V1) + idempotency (V2) + tick-freshness guards (V3)** | §8, P1-Phase-1 | ADD (V1/V3 HOLD-tier, V2 clean) | **HARD for I3 PROPOSE order actions** | one-click take/ticket flows would amplify the ungoverned-manual-path defect; I1/I2 display-only parts unaffected; RISK_STALE_TICK runs on the health/data fallback until V3 |
...
| 10 | `user_prefs` server table + cmdk palette activation | §11.18 + §4.2.5, P1-Phase-4 | ADD | DEGRADED | saved views/mutes ride localStorage interim (single-owner acceptable); server-side needed for §2.5 mutes to bind notification floors; §8.6 palette registrations wait for the palette (menu entries interim) |
...
| 19 | **Fired-side rail-operand side-channel** on signal emission (per-rail operands persisted for FIRED signals, mirroring `signal_rejections.diagnostic`) | new — surfaced by this design (§4.2); parity-safe additive per the golden side-channel rules | ADD | DEGRADED for §4.2 Stage-2 | fired-vs-rejected contrast runs Stage-1 only (composite + dot-supports); full per-rail contrast waits |
| 20 | **`notification_events` migration**: nullable `strategy_id` + `insight_id` column, + `artha.insights.delivery.*` channel config | new — surfaced by this design (§2.5); `strategy_id` is NOT NULL today | ADD (I1) | HARD for delivery-audited pushes of market/dataops-scoped insights | strategy-scoped insight pushes audit fine; non-strategy pushes would be in-app-only until the migration |
