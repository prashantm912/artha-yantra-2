...
**Status:** COMPLETE — ARCHIVED 2026-07-10 (all phases shipped+live; remaining forward-paper watch tracked in the ledger row `phase5-minervini-trend-template`) · **Created:** 2026-07-04 · **Owner:** solo (prashantm912)
...
> **Backtest-revised live values (2026-07-04/05, #556–#563 — these are the DECISION-TIME grill picks; the
> deep-history backtest later re-tuned several on evidence, see build-log + `docs/strategies/minervini-swing-backtest-results.md`):**
> `liquidity_multiple` **100→25** (turnover floor ₹37.5 L→₹9.375 L — the 100 default was a local minimum
> at every book size, v6); `max_concurrent` **4–8→12** (v7 sweet spot: best net CAGR + lower DD); `pilot_position_pct`
> **5–6%→6.5%** live (12×6.5%≈78% deployed, `max_deployment_pct` set to 80 as a DB risk row). RS-rank confirmed
> the single biggest edge (v3/v4); RS-rotation ruled out (v7). Live-published at strategy **v1.0.1**.
...
| MV-4.4 | `MinerviniCandidatePage.tsx` (analyzer, clone `BacktestResultsPage`) — tabs: Trend-Template (8 gates), Stage, VCP geometry, Fundamentals; lightweight-charts daily with 50/150/200 MA overlays + volume underlay; depends on the analyzer endpoint (MV-5.5) | NEW | MV-5.5 | **DONE (PR-O)** | `MinerviniCandidatePage` (`/equity/minervini/:symbol`) + `MinerviniCandidateChart` (candles + 50/150/200-day MA overlays + volume + dashed VCP-pivot line); 3 tabs; symbol links from screener + funnel. Verify trio green (lint/build tsc-strict/test:ci 257); deployed; live `/candidate` (DEEDEV 8/8, VCP `1W 12/7 4T`). | `MinerviniCandidatePage.tsx`, `MinerviniCandidateChart.tsx` |

### Phase 5 — VCP / base geometry + Stage detection (net-new detectors; Track B foundation)

| ID | Item | Grounding | Depends | Status | Verify | Evidence |
|---|---|---|---|---|---|---|
| MV-5.1 | Daily swing-high/low (zig-zag) extractor over a symbol's daily series — the primitive every base detector needs | NEW | MV-1.1 | **DONE (PR-D)** | `ZigZag` (pure, config threshold) + `ZigZagTest` (alternating swings + sub-threshold noise ignored) | `geometry/ZigZag.java` |
| MV-5.2 | **VCP detector** (§4.5/§4.6/§6.2): 2–6 contractions each ~½ the prior, volatility+volume contract left→right, final-contraction volume < 50d avg with 1–2 very-low days, pivot = tightest-contraction high; emits Technical Footprint `[W] [deep%/tight%] [count]T` | NEW | MV-5.1 | **DONE (PR-D)** | `VcpDetector` reproduces canonical **`40W 31/3 4T`**; rejects V-shape + expanding base; volume-dry-up baseline = 50d **as-of pivot** (adversarial-review fix); config `artha.minervini.vcp.*` | `geometry/VcpDetector.java` |
| ~~MV-5.3~~ | Stage label **moved to MV-2.9** (Phase 2 — it is a derived label, not geometry) | — | — | N/A | see MV-2.9 | |
| MV-5.4 | Persist per-candidate geometry — extend `minervini_screen_results` (or a sibling `minervini_setups` table) with stage (MV-2.9), footprint, pivot, base depth/duration, shakeout flags, base-count | NEW | MV-5.2, MV-2.9 | **DONE (PR-D)** | sibling **`V033__minervini_setups`** + `MinerviniSetupsRepository` + `MinerviniGeometryService.persistForPassers` (single writer, shared by scheduler + POST /run); IT round-trips an `is_vcp=true` row | `geometry/`, `MinerviniSetupsRepository.java` |
| MV-5.5 | Analyzer endpoint `GET /api/v1/market/screener/minervini/candidate/{symbol}` — Trend-Template booleans + Stage + VCP footprint + pivot + fundamentals grade (feeds MV-4.4) | NEW | MV-5.4 | **DONE (PR-D)** | typed `CandidateAnalysis` (persisted gates/stage/fundamentals + live geometry, `scanned` flag); spec recaptured + TS regen + `tsc --strict`; gateway `/api/v1/market/**` covers it | `MinerviniController.java` |
...
| ID | Item | Grounding | Depends | Status | Verify | Evidence |
|---|---|---|---|---|---|---|
| MV-6.1 | New engine indicators: ~~`WEEK52_HIGH`/`WEEK52_LOW`~~ (recon: 52-week band is a Track-A gate injected as context, not re-evaluated) → the **context-value family** `VCP_PIVOT`/`VCP_STAGE`/`RS_RANK_PCT`/`TREND_TEMPLATE_PASS` seeded from the screener (NEUTRAL in replay) | EXTEND `IndicatorRegistry` | MV-2.2, MV-5.2 | **PARTIAL (PR-F)** | **`VCP_PIVOT` DONE** (`contextLevel` alias; `IndicatorVectorTest` frozen vector; registry-freeze + schema enum synced; parity holds). `VCP_STAGE`/`RS_RANK_PCT`/`TREND_TEMPLATE_PASS` ride their first consumer (the funnel MV-6.7). | `IndicatorRegistry.java` |
| MV-6.2 | `session.style='swing'` in the strategy schema (daily primary, no square-off, multi-day hold) + engine handling | EXTEND (schema + `TickwiseGoldenRunner`/`SignalEngine`) | — | **DONE (PR-E)** | `SwingSessionTest`: 1d swing holds ≥2 sessions (entry, 0 exits); 1h intraday squares off; 1h swing does NOT (isolates the `style=swing` guard). schema enum +`swing`; `intervalDuration(1d)`; `SessionGate` `!swing`. All 9 golden + 9 parity vectors byte-identical. **Live `ROLLABLE_PRIMARIES`+`1d` / daily-rollup deferred to Phase 9; swing golden fixture to Phase 8 (MV-8.3).** | `TickwiseGoldenRunner.java`, `strategy-schema-v1.json` |
| MV-6.3 | `vcp` setup definition (breakout above pivot on expanding volume, don't-chase >~10% extended, 20d-MA post-breakout health) | NEW YAML | MV-6.1, MV-6.2 | **DONE (PR-F)** | `minervini-vcp.yaml` (`crossover(px,pivot)` = breakout+don't-chase in one; `vol>1.2`; `close>sma20`; swing 1d). `VcpSetupTest`: fires once on the breakout WITH volume; blocked WITHOUT (crossover fires but the volume gate stops it). Existing goldens byte-identical. **Interim `atr_multiple` stop — flat ~8% + staggered exits = Phase 7.** | `minervini-vcp.yaml`, `VcpSetupTest.java` |
| MV-6.4 | `cheat_3c` setup (buy above the cheat-pause high) | NEW YAML + `CHEAT_PIVOT` | MV-6.3 | **DONE (PR-J)** | `minervini-cheat-3c.yaml` (`crossover(px, cheat)` + `vol>1.2`, swing); `CheatPowerSetupTest` fires on the seeded cheat-pause breakout. Geometry that computes `cheat_pivot` (cheat-area high) rides Phase 9 seeding (like `VCP_PIVOT`). | `minervini-cheat-3c.yaml` |
| MV-6.5 | `power_play` setup (VCP-pivot breakout gated by a prior +100%/<8wk thrust) | NEW YAML + `THRUST` | MV-6.3 | **DONE (PR-J)** | `minervini-power-play.yaml` (`crossover(px, pivot)` + `vol>1.2` + `thrust>0`, swing); `CheatPowerSetupTest` fires WITH the thrust flag, blocked WITHOUT (thrust gate load-bearing). Thrust computed in Phase-9 seeding. | `minervini-power-play.yaml` |
| MV-6.6 | `primary_base` setup (new-high breakout; the post-IPO/first-base/depth filters are a candidate precondition, not an engine gate) | NEW YAML + `WEEK52_HIGH/LOW` | MV-6.3 | **DONE (PR-I)** | `WEEK52_HIGH/LOW` trailing-high/low indicators (exclude current bar → `close>w52h` = fresh high; `Week52BehaviorTest`); `minervini-primary-base.yaml` (`crossover(px,w52h)` + `vol>1.2`, swing); `PrimaryBaseSetupTest` fires on the new-high breakout WITH volume, blocked WITHOUT. Goldens byte-identical. | `SessionIndicators.java`, `minervini-primary-base.yaml` |
| MV-6.7 | `sepa` funnel — convergence of Trend-Template pass + RS-rank + a valid base + pivot-proximity; emits the ranked "immediately buyable / on-deck / watch" 3-list (§2.10) | NEW | MV-6.3 | **DONE (PR-G)** | `GET /api/v1/market/screener/minervini/funnel` (typed record) — joins the persisted screen + geometry, buckets by pivot proximity + valid VCP, ranks by RS. **Live-verified 2026-07-03: 62 buyable / 35 on-deck / 113 watch (=210 passers)**; DEEDEV/OMAXAUTO/NINSYS buyable at pivot. Impl = a market-data endpoint reusing Phase-5 rows (NOT an engine meta-strategy — simpler, live). *Market-regime gate (MV-6.9) + optional fundamentals fold in later.* | `MinerviniFunnelService.java` |
| MV-6.8 | `minervini_detail` side-channel (strategy lineage) — setup type, stage, footprint, pivot, gate booleans (mirrors `scalper_detail` V009) | NEW | MV-6.3 | **DONE (PR-R)** | `V020__minervini_signal_detail` (JSONB on `signals`, outside the frozen score_breakdown) + `SignalRepository.stampMinerviniDetail` + `SignalRow.minerviniDetail`. `MinerviniDetailIntegrationTest`: stamp→read round-trip (setup/stage/footprint), non-stamped rows carry null. The live PRODUCER that stamps it rides Phase 9 (its consumer). | `V020…sql`, `SignalRepository.java` |
...
| MV-7.2 | §2 risk framework — initial stop 7–8%, sizing, breakeven-at-3R, pyramid/never-average-down | EXTEND | MV-7.1 | **PARTIAL (PR-M)** | **flat-`percent` exit basis** in `ExitEvaluator` (= entry×value%; `premium_pct` aliased for equity) + schema enum; all 4 setups wired to `{basis: percent, value: 8}` (the locked initial stop). Verified: setups fire, `SwingBacktestTest` exits on the 8% stop, goldens byte-identical. Sizing `percent_equity` **6.5%** live (was 5%; backtest-tuned to a 12-slot / 80%-deployed book, #563). **Remaining:** breakeven-at-3R + pyramid; 50d-MA trail = a `trailing_stop`+`SMA(50)` config add (warms ≥50 daily bars). | `ExitEvaluator.java` |
| MV-7.3 | **Staggered stops** (§2.2) — partial closes at tiered stops (e.g. 4% half + 8% half ≈ 6%); a staggered-stop executor (multi-leg partial close, not single-precedence full close) | EXTEND `ExitEvaluator` | MV-7.2 | **DONE (PR-P) — same engine as MV-7.4** | The `scaled_exit` partial-close executor (below) IS the multi-leg engine; a staggered/tiered stop is expressible on the same fraction mechanism. Built parity-safe. | `ExitEvaluator.java`, `TickwiseGoldenRunner.java`, `ReplayEngine.java` |
| MV-7.4 | **Sell-into-strength / scaled profit exit** (§2.6/§3.6) — new exit rule type (tiers `[{profit_pct, qty_pct}]`), free-roll (sell half at 2–3R), 50-day-MA trailing stop | EXTEND schema + `ExitEvaluator` | MV-7.2 | **DONE (trail PR-Q + scaled PR-P)** | **50-day-MA trail:** `minervini-vcp-trail.yaml` + `SwingTrailTest`. **Scaled partial-close (PR-P):** `scaled_exit` rule — side-channel `qtyFraction` on `SignalEvent`/`ExitDecision` (frozen writer ignores it), `OpenPosition` remaining-fraction + fired-tiers + `applyExit` at the 3 runner sites, fraction-aware `ReplayEngine` legs (pro-rated entry cost, lot residual on the last leg). `ScaledExitTest` (2 partial exits of 0.5) + `ScaledBacktestTest` (2 fractional trades summing to the whole). **GoldenDeterminism 9/9 + BacktestParity 9/9 byte-identical**; full engine+backtest sweep 0 failures. | `minervini-vcp-scaled.yaml`, `ScaledExitTest.java`, `ScaledBacktestTest.java` |
...
| ID | Item | Grounding | Depends | Status | Verify | Evidence |
|---|---|---|---|---|---|---|
| MV-8.1 | **Screener hit-rate harness** (§13.8) — point-in-time `screen(asOf)` over a historical range (weekly), forward returns at +5/+10/+21/+63 sessions vs NIFTY, hit-rate + excess-return per horizon; **price-gates only** (no lookahead fundamentals — §3-E) | NEW (read-only SQL, market-data-service) | MV-2.1 | **DONE (PR-N)** | `MinerviniHitRateService` + `POST /screener/minervini/backtest` over `candles`@1d (deep history, NOT ~1y bhavcopy). Shared `MinerviniGates` (extracted from `TrendTemplateService` — one gate definition, no drift). **LIVE 2026-07-04 (3y/weekly, 149 steps, 104,523 samples):** mean excess +0.27%/+0.56%/+1.22%/**+2.92%** at +5/10/21/63; win% 48.8→53.0; asymmetric momentum edge (payoff in magnitude, not frequency). Each passer forward return measured to the SAME benchmark session date (adversarial-review fix); survivorship surfaced. `MinerviniHitRateIntegrationTest` 2/2. | `MinerviniHitRateService.java` |
| MV-8.2 | Swing-position backtest — feed the setups through `ReplayEngine` on a daily-bar primary, holding days→months | **FREE** (`ReplayEngine` drives `TickwiseGoldenRunner`, which already handles 1d/swing from PR-E) | MV-6.3 | **DONE (PR-L)** | `SwingBacktestTest`: the `vcp` swing setup replays to a **multi-day trade** (entry date < exit date, `barsHeld≥2`), held not squared-off, identical on two replays. Existing parity 9/9 green. | `SwingBacktestTest.java` |
| MV-8.3 | Swing-setup replay parity (determinism) | via `SwingBacktestTest` | MV-8.2 | **DONE (PR-L)** | two-replay trade-list equality; a committed `golden-minervini/` byte-freeze can be added later (optional) | `SwingBacktestTest.java` |
...
| ID | Item | Grounding | Depends | Status | Verify | Evidence |
|---|---|---|---|---|---|---|
| MV-9.1 | Live swing signal generation on the daily bar (screener → sepa funnel → published swing strategies → daily-bar entry eval); no intraday equity tick needed | REUSE engine | MV-6.7, MV-7.1 | **DONE (P9-C, #550)** | `MinerviniSwingEngine` daily batch (20:00 IST) — funnel candidates → per-symbol bank with seeded geometry → the FROZEN EntryEvaluator on the daily bar → emit ENTRY + `SignalEmitted` (auto-papered) + exit pass via ExitEvaluator. Funnel equities don't tick, so the tick engine (which refuses 1d primaries) never sees them — this batch is the driver. Goldens 9/9 byte-identical (no eval-core edits); 2-reviewer adversarial pass hardened it (auto-paper qty stamp + daily-close settle). Fires for real Monday post-close. | |
| MV-9.2 | **Selling discipline** (§3.6) — defensive (stop, close<20d-MA post-breakout, Stage-3/4 exit) + offensive (climax sell-into-strength, P/E-expansion + late base-count cues) as exit rules/gates | NEW YAML/gates | MV-7.4 | **DONE (PR-S) — defensive; offensive = scaled_exit** | `minervini-vcp-defensive.yaml` (`signal_exit crossunder(px, sma20)` — the defensive "close below the 20-day line" sell, existing `signal_exit`+`crossunder` engine, no parity surface); `SellingDisciplineTest` exits on the 20-MA break above the 8% stop; goldens byte-identical. **Offensive** climax sell-into-strength = the `scaled_exit` tiers (PR-P). **Stage-3/4 exit** = a context gate on a Stage indicator, deferred to Phase-9 seeding. | `minervini-vcp-defensive.yaml`, `SellingDisciplineTest.java` |
| MV-9.3 | Daily sell-decision list (§2.10 triad / §3.6.D) — for every open swing position, daily "would I buy now / why holding / where am I a seller" report | NEW | MV-9.1 | **DONE (P9-F, #553)** | `GET /api/v1/signals/minervini-swing/sell-decisions` — `MinerviniSellDecisionService` reuses the engine bank + frozen evaluators over each held anchor's fresh daily series (geometry from `minervini_detail`): buy-now (entry gate re-eval) / hold status (unrealized %) / where-seller (8% stop + 50d-MA trail + does an exit fire today). Read-only typed record. | |
| MV-9.4 | Notifier alerts — pivot pre-alert + stop alert + "candidate now immediately-buyable" push (reuse the notifier event→listener pattern; never import notifier from signals) | REUSE | MV-6.7 | **DONE-BY-REUSE (entry/stop); candidate-buyable = Phase-9** | **Already built:** `NotifierService.onSignal(SignalEmitted)` fires a per-strategy opt-in alert for ANY published strategy via `targetForVersion` — a published minervini swing strategy gets entry/stop alerts the moment the owner toggles notifications ON (no new code; a minervini-specific listener would be a redundant DUPLICATE — "verify before building"). **Net-new = the funnel "candidate-now-immediately-buyable" transition push**, which originates in market-data (the funnel) → a Phase-9 market-data producer + a cross-service push (deferred with the live producer). | (existing `NotifierService`) |
...
| ID | Item | Grounding | Depends | Status | Verify | Evidence |
|---|---|---|---|---|---|---|
| MV-10.1 | Post-analysis / trade grading (§2.11 Report Card: entry 60/20/15/5, exit-profit 70/15/15, exit-loss 65/20/15) over closed swing trades | NEW | MV-7.5 | **DONE (PR-U)** | `SwingReportCard.of(List<Trade>)` grades a closed-trade set A/B/C/D against the §0.5 #12 reliability bar (positive expectancy AND payoff ≥ 2 AND batting ≥ ~45%). `SwingReportCardTest`: a 2.5:1-payoff/60%-batting book grades A + passes; a losing book grades D; empty = N/A. Pure analytics, no parity surface. | `SwingReportCard.java` |
| MV-10.2 | Self-measurement (§2.9) — per-strategy avg win/loss, batting average, expectancy, avg hold-time; RBA stop calibration | NEW | MV-7.5 | **DONE (PR-U)** | Same `SwingReportCard`: batting avg, avg-win/avg-loss %, payoff ratio, expectancy %, avg bars-held — single-strategy (a run is one strategy, never blended); complements `MetricsCalculator` (which already yields winRate/profitFactor/expectancy). Computes over any run's `trades` (real consumer = the swing backtest); wiring onto the results endpoint is a trivial follow-up. | `SwingReportCard.java` |
...
| 2026-07-04 | MV-8.1 hit-rate harness (PR-N) | (build-log) | `MinerviniHitRateService` + `POST /screener/minervini/backtest` over `candles`@1d; shared `MinerviniGates` extracted (no screener drift). LIVE reliability evidence: passers' mean excess vs NIFTY is positive at every horizon and grows to +2.92% at +63 sessions (asymmetric momentum edge). 4-critic adversarial review → 2 fixes (benchmark-denominator consistency + stock/benchmark calendar alignment). |
| 2026-07-04 | Phase-9 LIVE pass (P9-A…P9-Z) | #548–#554 | Live Minervini = a daily EOD batch (`MinerviniSwingEngine`, 20:00 IST, flag-gated) reusing the FROZEN EntryEvaluator/ExitEvaluator over the fresh daily bar (funnel equities don't tick) → goldens 9/9 byte-identical. 4 setups seeded+published; auto-paper hold-across-sessions; sell-decision triad; buyable push. **Ran live on the 2026-07-04 (Fri) close → 8 entries → 8 paper positions open.** P9-G Stage-3/4 exit DEFERRED BY DOCTRINE. |
| 2026-07-04/05 | Deep-history backtest v1–v7 + live calibration | #556–#563 | ~11y event-driven sim over `candles`@1d (~1,789 EQ, ~40k signals) + 7-version A/B chain (`MinerviniSwingBacktest`/`SwingPortfolio`/`SwingRotationPortfolio`, V035; outside the parity firewall). Full write-up `docs/strategies/minervini-swing-backtest-results.md`. **Findings:** RS-rank is the edge (nearly doubles 8-slot CAGR — v3); RS-priority allocation = free +6–16pt (v4); ₹37.5 L turnover floor was a local minimum (v6); 12 slots is the sweet spot (v7); RS-rotation catastrophic (v7). **Live tuned (owner-confirmed):** `liquidity_multiple` 100→25 (#561); `max_open_paper_positions`→12; `max_deployment_pct`→80; sizing 5→6.5% (#563), re-published v1.0.1. Net expectation ~25% CAGR rs-turnover → ~39% rs-only, 40–50% DD. REMAINING = forward-paper watch + reliability sign-off. |
