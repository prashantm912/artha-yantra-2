...
public class ExpiredBackfillController {
   ...
   * {@code contractType} = {@code OPTIONS} / {@code FUTURES} / {@code BOTH}; null/blank ⇒ {@code BOTH}
   * (both option and future legs — the pre-selector default, byte-identical for existing callers).
   */
  ...
  public ResponseEntity<Map<String, String>> backfill(
    ...
    ExpiredBackfillRequest r =
        request == null ? new ExpiredBackfillRequest(null, null, null, null, null, null) : request;
    ...
    ExpiredBackfillService.ContractType contractType =
        ExpiredBackfillService.ContractType.parse(r.contractType());
    return ResponseEntity.status(HttpStatus.ACCEPTED)
        .body(service.triggerAsync(underlyings, from, to, r.interval(), force, contractType));
  }
