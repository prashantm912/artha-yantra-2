...
public class BhavcopyCloseCanary {
   ...
   *
   * <p>⚠️ Sweeps the LATEST bhavcopy date, which is not necessarily today's. That was near-harmless
   * while the schedule ran at 20:10 against a ~19:30 bhavcopy, but the evening chain now runs inside
   * 18:00–19:00 (the owner shuts the machine down at 19:00), and NSE's publish time varies — 17:52
   * and 17:59 on two measured days, 18:47 and 19:31 on two others. On a late night the bhavcopy has
   * not landed when this fires, and without the guard below the canary would silently re-evaluate
   * YESTERDAY's session and alert on it as if it were tonight's: an operator message that names the
   * wrong day, repeated every late night. Skip instead — a missed comparison is recoverable (the
   * next boot replays the chain), a confidently wrong one trains the owner to ignore the alert.
   */
