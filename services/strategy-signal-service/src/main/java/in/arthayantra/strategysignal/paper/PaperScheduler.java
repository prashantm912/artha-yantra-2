...
public class PaperScheduler {
  ...
  /**
   * 08:52 IST durable recovery of positions STRANDED past their expiry (audit AY-SL-04 follow-through):
   * an expiry-day settle that refused because the spot never ticked live would otherwise stay OPEN
   * forever ({@link PaperExpiryService#settleExpiries} acts only on the expiry date). Runs pre-open
   * (just after the 08:50 reconciler, on the same size-1 pool so it queues rather than races),
   * re-attempting settlement at the EXPIRY-SESSION close from
