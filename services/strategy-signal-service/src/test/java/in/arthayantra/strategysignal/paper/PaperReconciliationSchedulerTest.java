...
class PaperReconciliationSchedulerTest {
  ...
  private final SwingRunMutex swingRuns = mock(SwingRunMutex.class);
  ...
  @Test
  @DisplayName("the wait OBSERVES the swing lock, it never acquires it")
  void neverQueuesBehindTheCatchUp() {
    ...
    // while it stays a non-blocking observation, so the scheduler may touch anyRunInFlight() and
    // nothing else on the mutex — lockFor() would hand it a lock it could block on.
    when(swingRuns.anyRunInFlight()).thenReturn(false);
    when(reconciliation.reconcile()).thenReturn(cleanResult());
    ...
    verify(swingRuns, never()).lockFor(any());
  }
}
